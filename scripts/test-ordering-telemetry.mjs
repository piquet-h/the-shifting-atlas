...
async function testArtifactPruning() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        writeFileSync(join(TEST_ARTIFACTS_DIR, filename), JSON.stringify({
            issue: i,
            confidence: 'high',
...
async function testWeeklyMetrics() {
    ...
    const artifacts = [
        ...
        { issue: 3, confidence: 'medium', recommendedOrder: 3, applied: false, score: 100, metadata: { timestamp: new Date().toISOString() } },
        { issue: 4, confidence: 'low', recommendedOrder: 4, applied: false, score: 50, metadata: { timestamp: new Date().toISOString() } },
        { issue: 1, confidence: 'high', recommendedOrder: 5, applied: false, score: 150, metadata: { timestamp: new Date(Date.now() + 1000).toISOString() } } // Override within 24h
    ]
