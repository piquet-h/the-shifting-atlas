...
function validateSet(label, set) {
    ...
    for (const [k, v] of Object.entries(set.containers)) {
        if (!v) missing.push('COSMOS_SQL_CONTAINER_' + k.toUpperCase() + (label === 'test' ? '_TEST' : ''))
    }
    if (includeGremlin) {
...
function addTargets(label, set) {
    plan.targets.push({
        label,
        sqlEndpoint: set.sqlEndpoint,
        sqlDb: set.sqlDb,
        containers: set.containers,
        gremlin: includeGremlin ? { endpoint: set.gremlinEndpoint, db: set.gremlinDb, graph: set.gremlinGraph } : null
    })
}
...
if (invokeCli) {
    ...
    for (const t of plan.targets) {
        ...
        for (const [k, v] of Object.entries(t.containers)) {
            console.log(
                `az cosmosdb sql container delete --account-name <ACCOUNT_NAME_${t.label.toUpperCase()}> --resource-group <RG_${t.label.toUpperCase()}> --database-name ${t.sqlDb} --name ${v} --yes`
            )
        }
        if (includeGremlin) {
            console.log('\n# Gremlin Graph Delete (Option A – full graph):')
            console.log(
                `az cosmosdb gremlin graph delete --account-name <ACCOUNT_NAME_${t.label.toUpperCase()}> --resource-group <RG_${t.label.toUpperCase()}> --database-name ${t.gremlin.db} --name ${t.gremlin.graph} --yes`
            )
