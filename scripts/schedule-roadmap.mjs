...
async function fetchProjectItems() {
  ...
  for (const kind of attempts) {
    let hasNext = true; let after = null; const nodes = []; let projectId=null
    while (hasNext) {
      let data
      if (kind === 'viewer') {
        data = await ghGraphQL(`query($number:Int!,$after:String){viewer{projectV2(number:$number){id title items(first:100,after:$after){nodes{id content{... on Issue { id number title state createdAt closedAt labels(first:30){nodes{name}} }} fieldValues(first:50){nodes{ ... on ProjectV2ItemFieldDateValue { field { ... on ProjectV2FieldCommon { id name } } date } ... on ProjectV2ItemFieldSingleSelectValue { field { ... on ProjectV2FieldCommon { id name } } name optionId } } } pageInfo{hasNextPage endCursor}}}}}`,{number:PROJECT_NUMBER,after}).catch(e=>({viewer:null,_error:e}))
      } else {
        data = await ghGraphQL(`query($owner:String!,$number:Int!,$after:String){${kind}(login:$owner){projectV2(number:$number){id title items(first:100,after:$after){nodes{id content{... on Issue { id number title state createdAt closedAt labels(first:30){nodes{name}} }} fieldValues(first:50){nodes{ ... on ProjectV2ItemFieldDateValue { field { ... on ProjectV2FieldCommon { id name } } date } ... on ProjectV2ItemFieldSingleSelectValue { field { ... on ProjectV2FieldCommon { id name } } name optionId } } } pageInfo{hasNextPage endCursor}}}}}`,{owner:PROJECT_OWNER,number:PROJECT_NUMBER,after}).catch(e=>({[kind]:null,_error:e}))
      }
...
async function fetchProjectFields(projectId){
  const data = await ghGraphQL(`query($projectId:ID!){node(id:$projectId){... on ProjectV2 { fields(first:50){nodes{ ... on ProjectV2FieldCommon { id name } ... on ProjectV2DateField { id name } ... on ProjectV2SingleSelectField { id name options { id name } } }}}}}`,{projectId})
  return data.node.fields.nodes
}

async function updateDateField(projectId,itemId,fieldId,date){
  await ghGraphQL(`mutation($p:ID!,$i:ID!,$f:ID!,$d:Date!){updateProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f,value:{date:$d}}){projectV2Item{id}}}`,{p:projectId,i:itemId,f:fieldId,d:date})
}
...
function wholeDayDiff(a,b){ return Math.max(1, Math.round((b-a)/(1000*60*60*24))) }

function extractFieldValue(node, fieldName){ for(const fv of node.fieldValues.nodes){ if(fv.field?.name===fieldName){ return fv.date || fv.name || null } } return null }
...
function buildHistoricalDurations(projectItems, startFieldName, targetFieldName){
  ...
  for (const item of projectItems){
    ...
    let duration=null
    if(startStr && endStr){
      const s=new Date(startStr+'T00:00:00Z'); const e=new Date(endStr+'T00:00:00Z'); if(!isNaN(s)&&!isNaN(e) && e>=s) duration=wholeDayDiff(s,e)+0 // inclusive days
    }
    if(duration==null && content.createdAt && content.closedAt){
      const s=new Date(content.createdAt); const e=new Date(content.closedAt); if(!isNaN(s)&&!isNaN(e) && e>=s) duration=wholeDayDiff(s,e)
    }
    if(duration==null) continue
    const {scope,type}=classifyIssue(content)
    samples.push({scope,type,duration})
  }
  const byKey=new Map(); const byScope=new Map(); const all=[]
  for(const s of samples){
    const key=`${s.scope}|${s.type}`; if(!byKey.has(key)) byKey.set(key,[]); byKey.get(key).push(s.duration)
    if(!byScope.has(s.scope)) byScope.set(s.scope,[]); byScope.get(s.scope).push(s.duration)
    all.push(s.duration)
  }
