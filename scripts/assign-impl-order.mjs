...
The automation has analyzed this issue and calculated a recommended implementation order, but confidence is ${confidence} due to missing metadata.

### Recommendation
- **Recommended Order:** ${desiredMap.get(ISSUE_NUMBER)}
- **Priority Score:** ${computeScore(targetIssue)}
- **Missing:** ${missingMetadata.join(', ')}

### Current Issue Metadata
- **Scope:** ${scope === 'none' ? '❌ not set' : `✅ ${scope}`}
- **Type:** ${type === 'none' ? '❌ not set' : `✅ ${type}`}
- **Milestone:** ${milestone === 'none' ? '❌ not set' : `✅ ${milestone}`}

### Next Steps
To improve automation confidence, please add the missing labels/milestone. The automation will:
- ✅ **High confidence:** Automatically apply ordering
- ⚠️ **Medium confidence:** Require manual confirmation
- 🔴 **Low confidence:** Block auto-assignment
