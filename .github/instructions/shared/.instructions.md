...
- Backend/frontend must depend on `@piquet-h/shared` via the registry. Local `file:` references are not commit-safe and are checked by `verify:invariants`.
...
❌ **DO NOT** commit `"@piquet-h/shared": "file:../shared"` in package.json (warn-checked by `verify:invariants`)
