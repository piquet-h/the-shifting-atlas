...
- **Runtime:** Azure Functions Runtime v4 (Node.js LTS: 22.x preferred; 20.x still supported; 24.x not yet available in Functions – do NOT target 24 until GA). Pin Node version via `"engines"` in `package.json`.
...
- **Cosmos DB:** See `backend/AGENTS.md` (and `backend/src/persistenceConfig.ts`) for the current env-var/container mapping. Use Gremlin for immutable spatial graph; SQL API for mutable player/inventory/events. IDs always GUID. Avoid duplicating schema details here.
- **Service Bus:**
...
See `backend/AGENTS.md` for the current config key list. High-signal examples:

- `PERSISTENCE_MODE`
- `COSMOS_SQL_ENDPOINT` / `COSMOS_SQL_DATABASE` / `COSMOS_SQL_CONTAINER_*`
- `COSMOS_GREMLIN_ENDPOINT` (or legacy `COSMOS_ENDPOINT`)
- `ServiceBusAtlas__fullyQualifiedNamespace` (for `connection: 'ServiceBusAtlas'`)
- `APPLICATIONINSIGHTS_CONNECTION_STRING` (Functions host)
