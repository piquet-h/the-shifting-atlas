...
- **Cosmos DB:**
    - **Gremlin API**: Use for graph traversal (locations, exits, spatial relationships).
    - **SQL API**: Use `@azure/cosmos` SDK for document queries (players, inventory, layers, events).
    - Partition keys: players (`/id`), inventory (`/playerId`), layers (`/locationId`), events (`/scopeKey`).
    - All IDs are GUIDs; graph relationships stored as edges.
- **Service Bus:**
