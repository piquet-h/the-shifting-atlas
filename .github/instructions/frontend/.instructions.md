...
npm run format:frontend  # from repo root
# OR
npm run format          # from frontend/ directory
```
...
- Format: `npm run format:check:frontend` (repo root) OR `npm run format:check` (in `frontend/`) – must have no prettier errors
