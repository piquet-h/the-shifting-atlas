...
Replaces prior separate movement/performance workbook artifacts with a single consolidated dashboard:

- File: `infrastructure/workbooks/player-operations-dashboard.workbook.json`
- Infra: `infrastructure/workbook-player-operations-dashboard.bicep`
- Includes movement health (success/blocked, latency), Gremlin performance (RU/latency), and player operational indicators.
...
#### Operator Guidance Location

Operational triage guidance now lives primarily inside workbook markdown blocks (Player Operations, SQL Partition Monitoring, AI Operations) so on-call users get context where they are already investigating.

This document remains the canonical source for architecture intent, dashboard inventory, and governance rules. For ad-hoc data export or one-off analysis, use Azure Portal workbook export from the workbook editor and Log Analytics export from query results.
