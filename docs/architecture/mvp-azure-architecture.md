...
Status (2025-11-23): Player store cutover complete (ADR-004) – SQL API is authoritative for player state; Gremlin retains immutable world graph only (locations, exits, future NPC vertices). Strategic rationale in `overview.md`; milestone details in `../roadmap.md`.
...
## Dual Persistence Architecture (Post Cutover)

The system uses **dual persistence** (ADR-002, player aspects superseded by ADR-004): immutable world structure in Gremlin; mutable player/inventory/layer/event state in SQL API (player vertices removed).
...
export class MoveHandler {
    ...
    async movePlayer(playerId: string, direction: string) {
        ...
        const result = await this.locationRepo.move(playerId, currentLocationId, direction)
...
### Player Store Cutover (ADR-004)

The `DISABLE_GREMLIN_PLAYER_VERTEX` feature flag and all Gremlin fallback logic have been removed (see ADR-004). Player reads/writes use SQL API exclusively. Telemetry events related to migration & write-through were pruned (`Player.Migrate.*`, `Player.WriteThrough.*`, `Player.Get.Source*`). Remaining dual persistence applies only to the immutable world graph vs mutable document state.

Rollback (unlikely) would reintroduce the flag and fallback path per ADR-004 rollback section.
...
_Last updated: 2025-11-23 (player store cutover complete; flag & fallback removed per ADR-004)_
