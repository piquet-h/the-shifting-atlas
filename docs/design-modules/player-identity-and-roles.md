...
- Use Entra for sign-up/sign-in and federation (Microsoft, Google, Apple, etc.) and keep the canonical player profile in a durable player record linked by a stable external id such as the Entra `sub` claim or a custom claim.
- Validate ID tokens server-side in the backend identity boundary. Verify issuer, audience, expiry, and signature.
