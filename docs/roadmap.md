...
<!-- This document is regenerated by the scheduled workflow `.github/workflows/roadmap-scheduler.yml` and sync scripts (`npm run sync:impl-order:*`). -->
<!-- Manual edits (including status changes) will be reverted. Update `roadmap/implementation-order.json` or issue labels/status instead and let automation rebuild. -->
