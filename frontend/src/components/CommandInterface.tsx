...
export default function CommandInterface({ className, playerGuid }: CommandInterfaceProps): React.ReactElement {
  ...
  const runCommand = useCallback(async (raw: string) => {
    ...
    try {
      if (raw.startsWith('ping')) {
        ...
        const payload = { guid: playerGuid || 'guest', message: raw.replace(/^ping\s*/, '') || 'ping' };
  ...
  return (
    <div className={className}>
      ...
      <p className="mt-2 text-[10px] text-slate-500">Type <code>ping</code> to test latency, <code>clear</code> to reset.</p>
    </div>
  );
}
