 ...
 * Advisory only — must never be promoted to atlas tags automatically.
...
export interface EnvironmentalHintProposal {
    ...
    text: string
    /** Inferred compass direction, lower-case (e.g. `'north'`). */
    direction?: string
    /** Inferred terrain keyword, lower-case (e.g. `'hill'`). */
    terrainKind?: string
}
