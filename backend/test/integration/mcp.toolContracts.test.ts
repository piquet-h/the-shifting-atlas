...
import { getLocation, listExits } from '../../src/handlers/mcp/world/world.js'
...
describe('MCP Tool Contracts (World-*)', () => {
    let fixture: IntegrationTestFixture
    ...
    test('World-getLocation with valid locationId returns parsable JSON', async () => {
        const locationRepo = await fixture.getLocationRepository()
        const locationId = STARTER_LOCATION_ID
        ...
        const context = await fixture.createInvocationContext()
        const result = await getLocation({ arguments: { locationId } }, context)

        assert.doesNotThrow(() => JSON.parse(result), 'Result should be valid JSON')

        const parsed = JSON.parse(result)
        assert.ok(parsed, 'Should return location object')
        assert.strictEqual(parsed.id, locationId, 'Location ID should match')
        assert.strictEqual(parsed.name, 'Test Location', 'Location name should match')
    })

    test('World-getLocation with non-existent locationId returns null', async () => {
        const context = await fixture.createInvocationContext()
        const locationId = randomUUID()
    ...
    test('World-getLocation with omitted locationId defaults to starter', async () => {
        const locationRepo = await fixture.getLocationRepository()
        ...
        const context = await fixture.createInvocationContext()
        const result = await getLocation({ arguments: {} }, context)

        const parsed = JSON.parse(result)
        assert.strictEqual(parsed.id, STARTER_LOCATION_ID, 'Should default to starter location')
    })

    test('World-listExits returns parsable JSON with expected shape', async () => {
        const locationRepo = await fixture.getLocationRepository()
        const locationId = STARTER_LOCATION_ID
        ...
        // Create an exit via location repository (simpler than using HTTP handler)
        await locationRepo.upsert({
            id: locationId,
            name: 'Test',
            description: 'Test',
            exits: [{ direction: 'north', to: targetId }]
        })

        const context = await fixture.createInvocationContext()
        const result = await listExits({ arguments: { locationId } }, context)

        assert.doesNotThrow(() => JSON.parse(result), 'Result should be valid JSON')

        const parsed = JSON.parse(result)
        assert.ok(parsed.exits, 'Should have exits field')
        assert.ok(Array.isArray(parsed.exits), 'Exits should be array')
        assert.strictEqual(parsed.exits.length, 1, 'Should have 1 exit')
        assert.strictEqual(parsed.exits[0].direction, 'north', 'Exit direction should match')
    })

    test('World-listExits with no exits returns empty array', async () => {
        const locationRepo = await fixture.getLocationRepository()
        const locationId = STARTER_LOCATION_ID
        ...
        const parsed = JSON.parse(result)
        assert.ok(Array.isArray(parsed.exits), 'Should have exits array')
        assert.strictEqual(parsed.exits.length, 0, 'Should have empty array')
    })
})
